...
// IMPORTANT: sitemap index must ONLY reference www.jaipurcircle.com URLs.
// Never emit Supabase Edge URLs inside <loc>.
function firstPartySitemapUrl(type: string): string {
  switch (type) {
    case 'pages': return `${SITE_URL}/sitemap-pages.xml`
    case 'localities': return `${SITE_URL}/sitemap-localities.xml`
    case 'zones': return `${SITE_URL}/sitemap-zones.xml`
    case 'deals': return `${SITE_URL}/sitemap-deals.xml`
    case 'merchants': return `${SITE_URL}/sitemap-merchants.xml`
    case 'events': return `${SITE_URL}/sitemap-events.xml`
    case 'news': return `${SITE_URL}/sitemap-news.xml`
    case 'news-sitemap': return `${SITE_URL}/sitemap-news-google.xml`
    default: return `${SITE_URL}/sitemap.xml`
  }
}
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    return new Response(
      `<?xml version="1.0" encoding="UTF-8"?><error>Failed to generate sitemap: ${escapeXml(error?.message || String(error))}</error>`,
...
function generateSitemapIndex(today: string): string {
  // Only first-party URLs here (critical)
  const sitemaps = [
