  ...
  if (variant === "header") {
    return (
      <button
        onClick={onClick}
        className={cn(
          "flex items-center gap-1 px-2 py-1 rounded-full bg-primary/10 hover:bg-primary/20 transition-colors text-xs font-medium text-primary",
          className
        )}
      >
        <MapPin className="w-3 h-3 shrink-0" />
        <span className="max-w-[70px] truncate">{localityName}</span>
